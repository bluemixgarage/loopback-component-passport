  ...
  switch (authType) {
    ...
    case 'saml':
      ...
      function(req, profile, done) {
        // Use the samlExternalId field specified in the options
        // Azure saml profile returns http://schemas.microsoft.com/identity/claims/objectidentifier
        profile.id = profile.id || profile.nameID || profile[options.samlExternalId] || profile['http://schemas.microsoft.com/identity/claims/objectidentifier'];
        if (link) {
          if (req.user) {
  ...
  var defaultCallback = function(req, res, next) {
    ...
    passport.authenticate(name, _.defaults({session: session},
      ...
      if (!user) {
        ...
        // Use express-flash to report errors
        if (options.failureFlash) {
          if (typeof req.flash !== 'function') {
            next(new TypeError(g.f('{{req.flash}} is not a function')));
          }

          if (typeof options.failureFlash === 'string') {
            req.flash('error', options.failureFlash);
          } else {
