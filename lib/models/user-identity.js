      ...
      if (identity) {
        ...
        return identity.updateAttributes({profile: profile,
          ...
          return identity.user(function(err, user) {
            if (!err && user) {
              if (options.profileToUpdatedUser) {
                // Update user attributes
                var userObj = options.profileToUpdatedUser(provider, profile, options);
                user.updateAttributes(userObj, function(err, i) {
                  // Create access token if the autoLogin flag is set to true
                  if (!err && user && autoLogin) {
                    (options.createAccessToken || createAccessToken)(user, function(err, token) {
                      cb(err, user, identity, token);
                    });
                  }
                  else cb(err, user, identity);
                });
              }
              else cb(err, user, identity);
            }
